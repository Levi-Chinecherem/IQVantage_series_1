...
def generate_insights(data):
    ...
    # Check if any element in the Series is greater than 0
    if (pattern_detected > 0).any():
        pattern_name = "Two Crows (Bearish Reversal)"
    elif (pattern_detected < 0).any():
        pattern_name = "Bullish Reversal Pattern Detected"
    else:
        pattern_name = "No Pattern Detected"
    
    # Check for the "Head and Shoulders" pattern based on specific conditions
    # Recognize price patterns
    # Replace this with your own custom logic to detect the Head and Shoulders pattern
    def is_head_and_shoulders(data):
        # Implement your custom logic here to detect the Head and Shoulders pattern
        # Example: Check if the current pattern satisfies your criteria
        return (
            data['Close'].iloc[-1] < data['Close'].shift(-1).iloc[-1] and
            data['Close'].iloc[-1] < data['Close'].shift(-2).iloc[-1]
        )

    # ...

    # Inside generate_insights function
    # Check for the "Head and Shoulders" pattern based on your custom logic
    pattern_detected = is_head_and_shoulders(data)
...
def save_insights_as_pdf(insights, currency_pair):
    ...
    # Add images to the PDF with notes
    images = [
        ('insights/imgs/price_trends.png', 'Price Trends:'),
        ('insights/imgs/price_change_distribution.png', 'Price Change Distribution:'),
        ('insights/imgs/price_volatility.png', 'Price Volatility (20-day):')
    ]
    
    for image_path, note in images:
        pdf.add_page()
        pdf.image(image_path, x=10, y=50, w=190)
        pdf.set_font("Arial", size=12)
...
def main():
    ...
    # Save insights as a PDF with images and notes
